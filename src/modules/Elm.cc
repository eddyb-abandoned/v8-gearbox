...
static v8::Handle<v8::Value> _elm_Image_Image(const v8::Arguments &args) {
    Value This(args.This());
    if(args.Length() >= 2) {
        #line 278 "src/modules/Elm.gear"
        Value parent(args[0]), file(args[1]);
        Evas *evas = evas_object_evas_get(parent["eo"]);
